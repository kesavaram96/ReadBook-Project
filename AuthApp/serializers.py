...
class UserCreateSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if validated_data['password']==validated_data['password_confirmation']:
            user = User.objects.create(email=email,first_name=first_name,ROLE=ROLE,last_name=last_name,username=username)
            user.set_password(validated_data['password'])
            user.save()
            return user
        else:
            msg = _('Password must be same')
            raise serializers.ValidationError(msg)

    class Meta:
        model = User
        fields = ('email','id', 'password','username', 'first_name', 'last_name', 'ROLE','password_confirmation')
